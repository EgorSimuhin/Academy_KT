...
public:
    ...
    void move_smth(double T) {
        smth = 3 * sin(5*T) + 4 * cos(x*y) + 0.1*exp(T);
    }
    void move_velocity(double T) {
        vx = 1 * sin(2*(T-x/(vx+1)));
        vy = 1 * sin(3*(T-y/(vy+1)));
        vz = 0.01 * 3*sin(2*(T-x*z*y/(vz+1)));
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            //double vz = pow(4*cos(pointX) - sin(pointY), 2);
            //double vy = pow(1000*cos(pointX) - sin(pointY), 3);
            //double vx = cos(pointX)*10;
            // Модельная скалярная величина распределена как-то вот так
            double smth = 0;
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, 0.3, 0.1, 0);
        }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "Owl-step-" + std::to_string(snap_number) + ".vtu";
